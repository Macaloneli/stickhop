...
func _rollback_tick(delta: float) -> void:
	var avg_pos = (ik_target_left.global_position + ik_target_right.global_position) * 0.5
	var step_dest: Vector2 = Vector2.ZERO

	raycast.position.x = lerp(raycast.position.x, humanoid.state.direction * (25.0 if humanoid.state.crouch else 30.0), 3 * delta)
	...
	if humanoid.state.direction != 0:
		two_bone_left.flip_bend_direction = humanoid.state.direction < 0
		two_bone_right.flip_bend_direction = humanoid.state.direction < 0
	...
	elif humanoid.state.direction == 0 and humanoid.is_on_floor():
