...
func _physics_process(_delta: float) -> void:
	...
	humanoid.left_arm_point_vector = humanoid.get_global_mouse_position() if Input.is_action_pressed("point_left") else Vector2.ZERO
	humanoid.right_arm_point_vector = humanoid.get_global_mouse_position() if Input.is_action_pressed("point_right") else Vector2.ZERO
	if humanoid.left_arm_point_vector != Vector2.ZERO or humanoid.right_arm_point_vector != Vector2.ZERO:
		humanoid.grip = true
	else:
		humanoid.grip = false
