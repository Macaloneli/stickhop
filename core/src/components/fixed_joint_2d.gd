extends PinJoint2D
class_name FixedJoint2D

# https://gravityace.com/devlog/godot-fixed-joint/

var rotation_fix: float = 0
...
func connect_bodies(body1 : PhysicsBody2D, body2 : PhysicsBody2D):
	...
	body1.connect("tree_exiting", disconnect_joint)
	body2.connect("tree_exiting", disconnect_joint)
...
func _physics_process(_delta):
	...
	if node_b:
		var object: Node2D = get_node(node_b)
		if object:
