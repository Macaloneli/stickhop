...
func _start_button_pressed():
	...
	GamemodeManager.instance.load_gamemode(cur_choice)
	queue_free()
...
func _ready() -> void:
	...
	for item: GamemodeResource in gamemode_manager.registered_gamemodes:
		...
		new_item_button.info_icon.tooltip_text = (
			item.description + "\nLevels:\n%s" % _parse_level_list(item.levels)
		)
		new_item_button.pressed.connect(func(): _button_pressed(new_item_button, item))
